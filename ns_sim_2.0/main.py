...
from ns_sim.ui.app import run_ui
from ns_sim.sim.engine_ns import simulate_day, end_of_day_selection
...
def run():
    ap = argparse.ArgumentParser(description='Natural Selection mode (UI/headless)')
    ...
    for day in range(1, args.days + 1):
        ...
        for s in survivors:
            new_pop.append(Creature(id=s.id, species=s.species, speed=s.speed, size=s.size, sense=s.sense,
                                    x=0.0,y=0.0, home=(0.0,0.0), energy=0.0))
        for p in repro:
            new_pop.append(Creature(id=max([c.id for c in new_pop]+[0])+1, species=p.species,
                                    speed=p.speed, size=p.size, sense=p.sense,
                                    x=0.0,y=0.0, home=(0.0,0.0), energy=0.0))
        if len(new_pop) == 0:
            new_pop = _init_population(args.pop, start_id=1)
        population = new_pop
